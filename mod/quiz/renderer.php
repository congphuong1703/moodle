...
class mod_quiz_renderer extends plugin_renderer_base
{
    ...
    public function view_information($quiz, $cm, $context, $viewobj, bool $quizhasquestions = false)
    {
        ...
        $totalquestions = $viewobj->numattempts; // Tổng số câu hỏi, cần lấy từ quiz object hoặc tính toán.
        $totalpoints = $quiz->grade; // Tổng điểm, thường lưu trữ trong thuộc tính grade của quiz.
        $totaltime = format_time($quiz->timelimit);
        $output .= html_writer::div(
            html_writer::span('Số câu hỏi: ', 'font-weight-bold') . $totalquestions . ' Câu hỏi',
            'mb-2'
        );
