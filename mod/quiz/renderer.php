...
class mod_quiz_renderer extends plugin_renderer_base
{
    ...
    <div class="modal fade" id="askTeacherModal" tabindex="-1" aria-labelledby="askTeacherLabel" aria-hidden="true">
        <div class="modal-dialog">
            ...
            <form id="formrequestquestion" method="post" action="' . $CFG->wwwroot . '/theme/lms/requestquestion.php">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                            <input type="hidden" id="questionid" class="questionid" name="questionid">
                            <input type="hidden" id="next" name="next" value="' . $PAGE->url . '">
                            <textarea class="form-control" id="content" placeholder="Nội dung câu hỏi" name="content" rows="4" required></textarea>
                    </div>
                    <div class="modal-footer" style="text-align: right"> 
                        <button type="button" class="btn btn-outline-primary" data-dismiss="modal">Bỏ qua</button>
    ...
    public function view_page_guest($course, $quiz, $cm, $context, $messages, $viewobj)
    {
        ...
        $output .= html_writer::start_tag("div", array('class' => 'customize-attempt-content'));
        $output .= $this->view_page_tertiary_nav($viewobj);
     ...
     * @return string HTML to output.
     */
    public function view_information($quiz, $cm, $context, $viewobj, bool $quizhasquestions = false)
    {
        // Output any access messages.
        $output = "";
        $output .= html_writer::div('Cấu trúc đề thi:', 'font-weight-bold text-struct');
        $totalquestions = $viewobj->questioncount; // Tổng số câu hỏi, cần lấy từ quiz object hoặc tính toán.
        $totalpoints = $quiz->grade; // Tổng điểm, thường lưu trữ trong thuộc tính grade của quiz.
        $totaltime = format_time($quiz->timelimit);
        $output .= html_writer::start_div("struct-quiz mb-3");
        $output .= html_writer::div(
            html_writer::div('Tổng thời gian ') .
            html_writer::div($totaltime, 'font-weight-bold'),
            'mb-2'
        );
        $output .= html_writer::div(
            html_writer::div('Số câu hỏi ') . html_writer::div($totalquestions . ' Câu hỏi', 'font-weight-bold'),
            'mb-2'
        );
        $output .= html_writer::div(
            html_writer::span('Tổng điểm ') . html_writer::div(quiz_format_grade($quiz, $totalpoints) . ' Điểm', 'font-weight-bold'),
            'mb-2'
        );
        $output .= html_writer::end_div();

//            $output .= $this->box($this->access_messages($messages), 'quizinfo customize-struct');
//        $output .= html_writer::end_div();
