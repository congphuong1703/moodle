...
$teachers = $DB->get_records_sql("SELECT r.id, r.shortname 
        FROM {role_assignments} ra 
        JOIN {role} r ON ra.roleid = r.id 
        WHERE ra.userid = :userid ", ['userid' => $USER->id]);

$comments = $DB->get_records_sql("select c.id, c.customdiscussionid, c.content, c.userid, c.createdat, r.description as rolename, concat(u.firstname, ' ', u.lastname) as fullname 
from {custom_comments} c 
join {custom_discussions} d on c.customdiscussionid = d.id 
JOIN {user} u ON c.userid = u.id 
join {role_assignments} ra on ra.userid = u.id 
JOIN {role} r ON ra.roleid = r.id 
where c.customdiscussionid = :disccusstionid", ["disccusstionid" => $id]);
